...
public class RollControl extends AbstractControl {
	...
	protected void controlUpdate(float arg0) {
		...
		if (startRoll) {
			...
			diceControl.setPhysicsLocation(new Vector3f(-16, 2*(-2 + Integer.parseInt(spatial.getName())), 10));
			diceControl.setLinearVelocity(new Vector3f(12,  3*(2 - Integer.parseInt(spatial.getName())), 0));
...
//		}
		...
		if (step == -1 || (diceControl.getLinearVelocity().length() < 0.1f || diceControl.getAngularVelocity().length() < 1f)) {
			step = 0;
